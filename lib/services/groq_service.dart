...
///
/// URL proxy bisa diberikan saat build:
  ...
  Future<String> chat({
    ...
    if (proxyUrl.isEmpty) {
      throw const GroqException(
        'Glowy belum dikonfigurasi. Mohon set GLOWY_PROXY_URL saat build aplikasi ya, Bestie 💖',
      );
    }

    final messages = <Map<String, String>>[
      {'role': 'system', 'content': _systemPrompt},
      if (profile != null) {'role': 'system', 'content': _profileContext(profile)},
      ...history.map(
        (m) => {
    ...
    try {
      ...
      if (res.statusCode != 200) {
        throw GroqException(
          'Yahh, Glowy lagi ngambek nih (kode ${res.statusCode}). Coba lagi sebentar ya 🌸',
        );
      }
      ...
      throw GroqException(
        'Glowy belum bisa jawab sekarang ($err). Coba lagi ya, Bestie 💖',
      );
    } on TimeoutException {
      throw const GroqException(
        'Koneksi ke Glowy timeout. Coba cek internet kamu ya ✨',
      );
    } on GroqException {
      rethrow;
    } catch (_) {
      throw const GroqException(
        'Ups, ada error di koneksi Glowy. Coba lagi sebentar ya 🌸',
      );
    }
  }
